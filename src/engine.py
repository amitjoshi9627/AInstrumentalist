...
def _save_midi(midi_stream):
    ...
    def extractor(x): return x.split('.')[0].split('_')[-1]

    files = sorted([int(extractor(f)) for f in os.listdir(
        config.OUTPUT_MIDI) if f.endswith('.mid')])
    if files != []:
        index = files[-1]+1
...
def get_random_music():
    model = utils.load()
    X_val = utils.load_file(file_name="../saved_model/X_val.pickel")
    ...
    for i in range(config.PRED_ITERATIONS):
        ...
        random_music = np.insert(random_music[0], len(
            random_music[0]), prediction[0])
